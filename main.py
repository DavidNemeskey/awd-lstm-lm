...
# import model
import pytorch_lm.model as model_pkg
...
def train(train_data, model, criterion, optimizer, epoch, batch_size, ntokens,
          bptt, grad_clip, log_interval):
    ...
    num_steps = RandomNumSteps(bptt, 0.95, 5)
...
def main():
    ...
    # model = model_pkg.RNNModel(args.model, ntokens, args.emsize, args.nhid,
    #                            args.nlayers, args.dropout, args.dropouth, args.dropouti,
    #                            args.dropoute, args.wdrop, args.tied,
    #                            args.alpha, args.beta)
    model = model_pkg.RealMerityModel(
    ...
    try:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            train(model, train_data, criterion, optimizer, epoch,
                  args.batch_size, ntokens, args.bptt, args.clip,
                  args.log_interval)
            if 't0' in optimizer.param_groups[0]:
                ...
                val_loss2 = evaluate(val_data, model, criterion, ntokens, args.bptt)
                print('-' * 89)
                ...
                val_loss = evaluate(val_data, model, criterion, ntokens, args.bptt, eval_batch_size)
                print('-' * 89)
                print('| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.2f} | '
                      'valid ppl {:8.2f} | valid bpc {:8.3f}'.format(
    ...
    # Run on test data.
    test_loss = evaluate(test_data, model, criterion, ntokens, args.bptt, test_batch_size)
    print('=' * 89)
